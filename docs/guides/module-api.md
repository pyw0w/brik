...
## События и `onReady(client)` — продвинутые сценарии

`onReady(ctx)` получает сырой `client`. Это единственный санкционированный выход во внешний мир: здесь можно подписаться на gateway-события (входы/выходы участников, удаления/редакции сообщений, голос, баны). Модуль при этом **не обязан импортировать discord.js**: типы событий выводятся из перегрузок `client.on`, а эмбеды можно слать plain-объектами, структурно совместимыми с `APIEmbed`.

Пример — `src/modules/logs`: подписки в `onReady`, настройка каналов через `/logs` (право `ManageGuild`), конфиг в `store`. Требует привилегированных интентов (`GuildMembers`, `MessageContent`) в настройках приложения Discord.

**Правило**: подписки должны быть самочитаемы — `client.on(...)` регистрируются в `onReady`, доставка в канал — fire-and-forget с тихим fallback, ошибки в логгер.
