...
**Log**:
Категория событий логирования (мод-действия, участники, сообщения, голос, наказания), которой на сервере сопоставлен канал через `/logs`. Живёт в `src/modules/logs` (подписки на gateway-события в `onReady`).
_Avoid_: журнал, логгер (как отдельная сущность)
